...
/* 
Julia and Kate are still studying dogs. This time they are want to figure out if the 
dogs in their are eating too much or too little food.
...
above and below the recommended portion (see hint).
...
1. Loop over the array containing dog objects, and for each dog, calculate the
 recommended food portion (recFood) and add it to the object as a new property.
  Do NOT create a new array, simply loop over the array (We never did this before, so 
  think about how you can do this without creating a new array).
2. Find Sarah's dog and log to the console whether it's eating too much or too little.
 HINT: Some dogs have multiple users, so you first need to find Sarah in the owners 
 array, and so this one is a bit tricky (on purpose) 🤓
3. Create an array containing all owners of dogs who eat too much (ownersTooMuch) and an
 array with all owners of dogs who eat too little (ownersTooLittle).
...
6. Log to the console whether ALL of the dogs are eating an OKAY amount of food (just 
true or false)
...
10. Sort the dogs array by recommended food portion in an ascending order. Make sure 
...
for (let dog of dogs) {
    ...
    const recomendedFood = Math.round(dogWeight ** 0.75 * 28);
    dog.recFood = recomendedFood;
}
console.log(dogs);

//2
const SarahDogsDiet = dogs.map(d => d.owners).filter(arr => arr.includes('Sarah'));
const SarahDogsIndex = dogs.findIndex(d => d.owners.includes('Sarah'));
console.log(dogs[SarahDogsIndex].curFood > dogs[SarahDogsIndex].recFood ? 'Over Food Eating' : 'Under Food Eating');
console.log(SarahDogsIndex);
...
//5 
console.log(dogs.map(dog => dog.curFood === dog.recFood));
